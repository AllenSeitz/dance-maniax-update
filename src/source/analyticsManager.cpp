...
void AnalyticsManager::logEvent(const char* eventCategory, const char* eventType, const char* eventLabel, int eventValue)
{
	...
	char postfields[512] = "";
	sprintf_s(postfields, 512, "v=%d&tid=%s&an=%s&cid=%s&t=%s&ec=%s&ea=%s", 1, TRACKING_ID, TRACKING_APP_NAME, clientID, "event", eventCategory, eventType);
	if ( eventLabel[0] != 0 )
	{
	...
	if ( curl )
	{
	    curl_easy_setopt(curl, CURLOPT_URL, GOOGLE_BASE_URL);
